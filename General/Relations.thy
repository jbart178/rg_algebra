...
lemma domain_restrict_inter: "p \<triangleleft> (q1 \<inter> q2) = p \<triangleleft> q1 \<inter> q2"
  by (simp add: inf_assoc restrict_domain_def)

lemma range_restrict_inter: "(q1 \<inter> q2) \<triangleright> p = q1 \<triangleright> p \<inter> q2"
  by (simp add: boolean_algebra_cancel.inf1 inf_commute restrict_range_def)
...
lemma range_restrict_remove: "r \<triangleright> p \<subseteq> r"
  using restrict_range_def by auto

lemma domain_range_restrict_remove: "p1 \<triangleleft> r \<triangleright> p2 \<subseteq> r"
  using domain_restrict_remove range_restrict_remove by blast
...
lemma domain_restricts_contains: 
  assumes "\<And>\<sigma> \<sigma>'. p \<sigma> \<and> q1 \<sigma> \<sigma>' \<longrightarrow> q2 \<sigma> \<sigma>'"
  shows "{\<sigma>. p \<sigma>} \<triangleleft> {(\<sigma>,\<sigma>'). q1 \<sigma> \<sigma>'} \<subseteq> {(\<sigma>,\<sigma>'). q2 \<sigma> \<sigma>'}"
proof -
  have a: "{\<sigma>. p \<sigma>} \<triangleleft> {(\<sigma>,\<sigma>'). q1 \<sigma> \<sigma>'} = {(\<sigma>,\<sigma>'). p \<sigma> \<and> q1 \<sigma> \<sigma>'}"
    using restrict_domain_def by auto 
  have b: "\<dots> \<subseteq> {(\<sigma>,\<sigma>'). q2 \<sigma> \<sigma>'}"
    using assms by blast 
  show ?thesis using a b by presburger
qed

lemma range_restricts_contains:
  assumes "\<And>\<sigma> \<sigma>'. q1 \<sigma> \<sigma>' \<and> p \<sigma>' \<longrightarrow> q2 \<sigma> \<sigma>'"
  shows "{(\<sigma>,\<sigma>'). q1 \<sigma> \<sigma>'} \<triangleright> {\<sigma>. p \<sigma>} \<subseteq> {(\<sigma>,\<sigma>'). q2 \<sigma> \<sigma>'}"
proof -
  have a: "{(\<sigma>,\<sigma>'). q1 \<sigma> \<sigma>'} \<triangleright> {\<sigma>. p \<sigma>} = {(\<sigma>,\<sigma>'). q1 \<sigma> \<sigma>' \<and> p \<sigma>'}"
    using restrict_range_def by auto 
  have b: "\<dots> \<subseteq> {(\<sigma>,\<sigma>'). q2 \<sigma> \<sigma>'}"
    using assms by blast 
  show ?thesis using a b by presburger
...
lemma stable_antimono_r:
  assumes stable_r1: "stable p r2"
  assumes subset: "r1 \<subseteq> r2"
  shows "stable p r1"
proof -
  have b: "r1``p \<subseteq> r2``p"
    using subset by (simp add: Image_mono)
  show ?thesis using stable_r1 by (meson b dual_order.trans stable_def) 
qed
...
(* Lemmas about interference, and tolerating interference. *)
...
lemma tolerates_interference_restrict_post:
  assumes tolerate: "tolerates_interference p q r"
