...
theory Conjunction
imports Refinement_Lattice Abstract_Sync Galois_Connections 
begin
...
locale conj_distrib = conjunction + abstract_sync conj + upper_galois_connections
begin
...
lemma conj_lfp_distrib:
  assumes monoG: "mono G"
  assumes nonaborting: "chaos \<ge> c"
  assumes conj_G_distrib: "\<And>x. (c \<iinter> G x = c \<iinter> G(c \<iinter> x))"
  shows "c \<iinter> lfp G = lfp (\<lambda>x. c \<iinter> G x)"
proof -
  define F where "F \<equiv> \<lambda>x. c \<iinter> x"
  define H where "H \<equiv> \<lambda>x. c \<iinter> G x"
  have monoH: "mono H" 
    by (metis H_def dual_order.refl monoG mono_def sync_mono)
  have distF: "dist_over_nondet F" 
    using F_def conj_Nondet_distrib_nonaborting nonaborting by blast 
  have fog: "\<And>x. (F \<circ> G)x = (H \<circ> F)x"
    using F_def H_def conj_G_distrib by auto
  show ?thesis using fusion_lfp_eq monoH distF fog 
    using F_def H_def monoG by blast
qed
