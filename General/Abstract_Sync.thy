...
lemma non_aborting_distrib: 
  assumes "c \<otimes> \<bottom> = \<bottom>"
  shows "c \<otimes> (\<Squnion>D) = (\<Squnion>d \<in> D. c \<otimes> d)"
proof -
  show ?thesis by (metis Sup_empty assms image_empty sync_Nondet_distrib)
qed

lemma non_aborting_distrib2: 
  assumes "c \<otimes> \<bottom> = \<bottom>"
  shows "c \<otimes> (\<Squnion>d\<in>D. f(d)) = (\<Squnion>d \<in> D. c \<otimes> f(d))"
proof -
  show ?thesis
    by (metis Sup_empty assms image_empty sync_NONDET_distrib)
qed
