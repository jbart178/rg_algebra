...
lemma iter_to_iter_plus_iter:
  shows "c\<^sup>\<omega> = (c ; c\<^sup>\<omega>)\<^sup>\<omega>"
proof (rule antisym)
  have one_iter: "c ; c\<^sup>\<omega> \<ge> c"
    by (metis iter0 seq_mono_right seq_nil_right)
  then show "(c ; c\<^sup>\<omega>)\<^sup>\<omega> \<ge> c\<^sup>\<omega>"
    by (metis one_iter iter_mono)
next
  have induct_goal: "nil \<squnion> c ; (c ; c\<^sup>\<omega>)\<^sup>\<omega> \<ge> (c ; c\<^sup>\<omega>)\<^sup>\<omega>"
    by (metis iter_decomp iter_unfold seq_assoc seq_nondet_distrib_weak sup_idem)
  then show "c\<^sup>\<omega> \<ge> (c ; c\<^sup>\<omega>)\<^sup>\<omega>"
    by (metis induct_goal iter_induct_nil)
qed
...
(*
lemma fiter_unfold_right: "c\<^sup>\<star> = nil \<squnion> c\<^sup>\<star>;c"
proof -
  have "c\<^sup>\<star> = nil \<squnion> c ; (\<Squnion>i::nat. (c \<^sup>;^ i))"
    using fiter_seq_choice fiter_unfold by auto
  also have "... = nil \<squnion> (\<Squnion>i::nat. (c \<^sup>;^ Suc i))"
    by (metis NONDET_nat_shift fiter_seq_choice fiter_seq_choice_nonempty)
  also have "... = nil \<squnion> (\<Squnion>i::nat. (c \<^sup>;^ i) ; c)"
    by (simp add: seq_power_front)
  also have "... = nil \<squnion> (\<Squnion>i::nat. c \<^sup>;^ i) ;c"
    by (simp add: NONDET_seq_distrib)
  finally show ?thesis 
    by (metis fiter_seq_choice)
qed
*)
