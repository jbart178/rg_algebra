...
lemma iter_i: "c\<^sup>\<omega> \<ge> c \<^sup>;^ i"
proof (induct i)
  case 0
  show ?case 
    by (metis iter0 seq_power_0)
next
  case (Suc i)
  have c_suc_i: "c;c\<^sup>\<omega> \<ge> c \<^sup>;^ (Suc i)"
    by (simp add: Suc seq_mono_right)
  have "c\<^sup>\<omega> = nil \<squnion> c;c\<^sup>\<omega>"
    by (metis iter_unfold)
  also have "... \<ge> c \<^sup>;^ (Suc i)"
    using c_suc_i le_supI2 by blast
  finally show ?case .
qed
...
lemma fiter_i: "c\<^sup>\<star> \<ge> c \<^sup>;^ i"
proof (induct i)
  case 0
  show ?case 
    by (metis fiter0 seq_power_0)
next
  case (Suc i)
  have c_suc_i: "c;c\<^sup>\<star> \<ge> c \<^sup>;^ (Suc i)"
    by (simp add: Suc seq_mono_right)
  have "c\<^sup>\<star> = nil \<squnion> c;c\<^sup>\<star>"
