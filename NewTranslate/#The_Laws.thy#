...
lemma local_ass:
  assumes single_reference_e: "\<forall>(r :: Value*Value \<Rightarrow> bool). single_reference e \<llangle>r (\<ordfeminine>x, \<ordmasculine>x)\<rrangle>"
  assumes estable_e: "(r :: Value*Value \<Rightarrow> bool) \<Longrightarrow> estable e \<llangle>r\<rrangle>"
  assumes stable: "stable \<lrel>p\<rrel> \<lpred>r\<rpred>"
