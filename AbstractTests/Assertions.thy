...
lemma assert_alt_def: "assert p = nil \<squnion> test.negate(\<tau> p);\<top>"
proof -
  have a: "test.negate(\<tau> p);\<top> \<ge> test.negate(\<tau> p)" 
    by simp 
  have b: "nil \<squnion> test.negate(\<tau> p);\<top> = \<tau> p \<squnion> test.negate(\<tau> p) \<squnion> test.negate(\<tau> p);\<top>" 
    using test_nondet_compl by auto
  then have c: "\<dots> = \<tau> p \<squnion> test.negate(\<tau> p);\<top>" using a 
    by (simp add: inf_sup_aci(6) sup.absorb_iff2)
  then show ?thesis 
    using assert_def b by presburger
qed
...
lemma assert_remove_left: "\<lbrace>p\<rbrace>;c \<ge> c"
  using assert_redundant by blast

lemma assert_remove: "c;\<lbrace>p\<rbrace>;d \<ge> c;d"
  using assert_remove_left seq_assoc seq_mono_right by force
