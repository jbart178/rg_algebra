...
lemma sync_nil_magic:
  assumes "c \<otimes> nil = \<bottom>"
  shows "c \<otimes> \<bottom> = \<bottom>"
  by (metis assms comm.left_commute test.hom_bot test_sync_idem test_sync_nil)
end
...
lemma test_command_sync_command:
  assumes t_def[simp]: "t = (\<tau> p)" (* For convenience of notation *)
  assumes c_nonabort: "test.negate t ; c \<otimes> \<bottom> = \<bottom>" 
  shows "c \<otimes> t ; d = t ; (c \<otimes> d)"
proof -
  have "t ; (c \<otimes> t ; d) = t ; c \<otimes> t ; t ; d"
    by (simp add: seq_assoc test_sync_distrib)
  have t_prefix: "... = t ; (c \<otimes> d)"
    using test_seq_idem test_sync_distrib by auto
  have not_t_prefix: "test.negate t ; c \<otimes> test.negate t ; t ; d = \<bottom>"
    using c_nonabort test_seq_commute test_seq_compl by force
  have "c \<otimes> t ; d = (t \<squnion> test.negate t) ; (c \<otimes> t ; d)"
    using test_nondet_compl by force
  also have "... = t ; (c \<otimes> t ; d) \<squnion> test.negate t ; (c \<otimes> t ; d)"
    by (simp add: nondet_seq_distrib)
  also have "... = t ; (c \<otimes> d) \<squnion> \<bottom>"
    using not_t_prefix seq_assoc t_prefix test_sync_distrib by fastforce
  also have "... = t ; (c \<otimes> d)"
    by simp
  finally show ?thesis .
qed
