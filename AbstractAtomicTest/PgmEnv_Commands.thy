...
lemma conj_infinite:
  shows "c \<iinter> Atomic\<^sup>\<infinity> = c ; \<bottom>"
proof -
  have "c \<iinter> Atomic\<^sup>\<infinity> = c \<iinter> (Atomic\<^sup>\<omega> ; \<bottom>)"
    by (simp add: infiter_iter_magic)
  also have "... = c \<iinter> (chaos ; \<bottom>)"
    by (simp add: chaos_def)
 also have "... = c \<iinter> (chaos ; \<tau> bot)"
    by (simp)
  also have "... = (c \<iinter> chaos) ; \<tau> bot"
    by (metis conj.test_sync_post)
  also have "... = c ; \<bottom>"
    by (simp)
  finally show ?thesis .
qed
...
lemma par_infinite:
  shows "c \<parallel> \<E>\<^sup>\<infinity> = c ; \<bottom>"
proof -
  have "c \<parallel> \<E>\<^sup>\<infinity> = c \<parallel> (\<E>\<^sup>\<omega> ; \<bottom>)"
    by (simp add: infiter_iter_magic)
  also have "... = c \<parallel> (skip ; \<bottom>)"
    by (simp add: skip_def)
 also have "... = c \<parallel> (skip ; \<tau> bot)"
    by (simp)
  also have "... = (c \<parallel> skip) ; \<tau> bot"
    by (metis par.test_sync_post)
  also have "... = c ; \<bottom>"
    by (simp)
  finally show ?thesis .
qed

lemma conj_par_swap_inf: "c \<iinter> Atomic\<^sup>\<infinity> = c \<parallel> \<E>\<^sup>\<infinity>" by (metis conj_infinite par_infinite)

lemma sync_inf_swap:
  fixes sync :: "'a \<Rightarrow> 'a \<Rightarrow> 'a" (infixl "\<otimes>" 75)
  assumes "sync = par \<or> sync = conj"
  shows "(c\<^sup>\<omega> \<iinter> Atomic\<^sup>\<infinity>) \<otimes> d = c\<^sup>\<omega> \<otimes> (Atomic\<^sup>\<infinity> \<iinter> d)"
proof (cases "sync = conj")
  case True
  then show ?thesis
    by (simp add: local.conj_assoc)
next
  case False
  have sync_par: "sync = par"
    using False assms by auto
  have "(c\<^sup>\<omega> \<iinter> Atomic\<^sup>\<infinity>) \<otimes> d = (c\<^sup>\<omega> \<iinter> Atomic\<^sup>\<infinity>) \<parallel> d"
    by (metis sync_par)
  also have "... = (c\<^sup>\<omega> \<parallel> \<E>\<^sup>\<infinity>) \<parallel> d"
    by (metis conj_par_swap_inf)
  also have "... = c\<^sup>\<omega> \<parallel> (\<E>\<^sup>\<infinity> \<parallel> d)"
    by (metis par_assoc)
  also have "... = c\<^sup>\<omega> \<parallel> (Atomic\<^sup>\<infinity> \<iinter> d)"
    by (metis conj_par_swap_inf conj_commute par_commute)
  also have "... = c\<^sup>\<omega> \<otimes> (Atomic\<^sup>\<infinity> \<iinter> d)"
    by (metis sync_par)
  finally show ?thesis .
qed

lemma inf_as_conj: "c\<^sup>\<infinity> = c\<^sup>\<omega> \<iinter> Atomic\<^sup>\<infinity>" by (metis infiter_iter_magic conj_infinite)

lemma Atomic_conj_atomic_finite:
  assumes "a = atomic(g, r)"
  shows "a \<^sup>;^ j = (Atomic \<^sup>;^ j) \<iinter> (a \<^sup>;^ j)"
proof (induct j)
  case 0
  show "a \<^sup>;^ 0 = (Atomic \<^sup>;^ 0) \<iinter> (a \<^sup>;^ 0)"
    by (simp add: conj.nil_sync_nil)
next
  case (Suc j)
  have "a \<^sup>;^ (Suc j) = a ; (a \<^sup>;^ j)"
    by (simp)
  also have "... = (Atomic \<iinter> a) ; ((Atomic \<^sup>;^ j) \<iinter> (a \<^sup>;^ j))"
    by (metis (full_types) Atomic_def assms conj.atomic_power_pre_sync_power_pre conj.atomic_pre_sync_atomic_pre par.conj_atomic_sync_identity seq_power_front)
  also have "... = (Atomic ; (Atomic \<^sup>;^ j)) \<iinter> (a ; (a \<^sup>;^ j))"
    by (metis Atomic_def assms conj.atomic_pre_sync_atomic_pre)
  finally show ?case 
    by simp
qed
