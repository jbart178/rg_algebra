...
locale constrained_atomic = constrained_atomic_pre + guarantees +
...
proof -
  have a: "guar  g \<iinter> opt q = ((guar g) \<iinter> \<pi> q) \<squnion> ((guar g) \<iinter> \<tau>({\<sigma>. (\<sigma>,\<sigma>)\<in>q }))"
    using opt_def conj.sync_nondet_distrib opt_def by presburger
  then have b: "\<dots> = \<pi>(g \<sqinter> q) \<squnion> \<tau>({\<sigma>. (\<sigma>,\<sigma>)\<in>q })"
    using guar_conj_pgm guar_conj_test by presburger 
  then have c: "\<dots> = \<pi>(g \<sqinter> q) \<squnion> \<tau>({\<sigma>. (\<sigma>,\<sigma>)\<in> g \<sqinter> q })"
    using refl_g by (simp add: refl_onD) 
  show ?thesis using a b c opt_def by force
qed
