...
proof -
  have "tolerates_interference p (\<top> \<triangleright> (b0 \<inter> p)) r"
    using assms stable_inter tolerates_interference_def tolerates_interference_restrict_post top_greatest by metis
...
proof -
  ...
  also have "... \<le> \<lbrakk>b\<rbrakk>true ; (rely r \<iinter> guar g \<iinter> \<lbrace>p \<inter> b0\<rbrace> ; term ; \<tau> (-t) ; \<lparr>\<top> \<triangleright> (expr_eq b false \<inter> p)\<rparr> ; idle) ; \<bottom>"
  proof -
    have "\<And>x. (guar g \<iinter> x) ; idle \<le>  guar g \<iinter> x ; idle"
      using \<open>refl g\<close> guar_distrib_seq guar_idle by metis
    thus ?thesis
      using conj_assoc by (simp add: conj.sync_mono seq_mono_left seq_mono_right)
  qed
