...
lemma guar_conj_assert: "(guar g) \<iinter> \<lbrace>p\<rbrace> = \<lbrace>p\<rbrace>"
  using assert_def guar_conj_test by (metis conj.nil_sync_assert local.conj_assoc test_top)

lemma guar_conj_pgm: "(guar g) \<iinter> \<pi> q = \<pi>(g \<sqinter> q)"
proof -
  have a: "guar g \<iinter> \<pi> q = (\<pi>(g) \<squnion> \<E>)\<^sup>\<omega> \<iinter> \<pi> q" 
    by simp
  then have b: "\<dots> = (nil \<squnion> (\<pi>(g) \<squnion> \<E>);(\<pi>(g) \<squnion> \<E>)\<^sup>\<omega>) \<iinter> \<pi> q" 
    using iter_unfold by auto
  then have c: "\<dots> = (nil \<iinter> \<pi> q) \<squnion> ((\<pi>(g) \<squnion> \<E>);(\<pi>(g) \<squnion> \<E>)\<^sup>\<omega> \<iinter> \<pi> q;nil)" 
    using conj.nondet_sync_distrib seq_nil_right by presburger
  then have d: "\<dots> = \<bottom> \<squnion> (\<pi>(g) \<squnion> \<E>) \<iinter> \<pi> q"
    by (metis conj.atomic_iter_sync_nil conj.atomic_pre_sync_atomic_pre conj.nil_sync_atomic_pre pgm_atomic pgm_or_Env_atomic seq_nil_right)
  then have e: "\<dots> = \<pi>(g \<sqinter> q) \<squnion> (\<E> \<iinter> \<pi> q)" 
    using conj.nondet_sync_distrib pgm_conj_pgm by auto
  show ?thesis using a b c d e 
    by (simp add: Env_def env_conj_pgm)
qed
