...
locale sync_iteration = abstract_sync + iteration_infinite_distrib +
  ...
  assumes sync_seq_weak_interchange: "d\<^sub>1 ; d\<^sub>2 \<otimes> c\<^sub>1 ; c\<^sub>2 \<ge> (d\<^sub>1 \<otimes> c\<^sub>1) ; (d\<^sub>2 \<otimes> c\<^sub>2)"
begin

lemma sync_distrib_seq:
  assumes "d \<ge> d ; d"
  shows "d \<otimes> c\<^sub>1 ; c\<^sub>2 \<ge> (d \<otimes> c\<^sub>1) ; (d \<otimes> c\<^sub>2)"
proof -
  have a1: "d \<otimes> c\<^sub>1 ; c\<^sub>2 \<ge> d ; d \<otimes> c\<^sub>1 ; c\<^sub>2"
    by (simp add: assms sync_mono)
  have a2: "d ; d \<otimes> c\<^sub>1 ; c\<^sub>2 \<ge> (d \<otimes> c\<^sub>1) ; (d \<otimes> c\<^sub>2)"
    by (simp add: sync_seq_weak_interchange)
  show ?thesis using a1 a2 by force
qed

lemma distrib_fixed_iter:
...
next
  case (Suc i)
  have "d \<otimes> (c \<^sup>;^ Suc i) = d \<otimes> c ; (c \<^sup>;^ i)"
    by (metis seq_power_Suc)
  also have "... = (d \<otimes> c) ; (d \<otimes> (c \<^sup>;^ i))"
    by (simp)
  also have "... = (d \<otimes> c) ; ((d \<otimes> c) \<^sup>;^ i)"
    by (metis Suc.hyps)
  thus ?case by simp
...
lemma sync_fixed_iter: "(atomic p \<^sup>;^ i) ; c \<otimes> (atomic q \<^sup>;^ i) ; d = (((atomic p) \<otimes> (atomic q)) \<^sup>;^ i) ; (c \<otimes> d)"
proof (induct i)
  case 0
  show ?case
    by simp
next
  case (Suc i)
  have "(atomic p \<^sup>;^ Suc i) ; c \<otimes> (atomic q \<^sup>;^ Suc i) ; d = atomic p ; (atomic p \<^sup>;^ i) ; c \<otimes> atomic q ; (atomic q \<^sup>;^ i) ; d"
    by simp
  also have "... = (atomic p \<otimes> atomic q) ; ((atomic p \<^sup>;^ i) ; c \<otimes> (atomic q \<^sup>;^ i) ; d)"
    by (metis atomic_pre_sync_atomic_pre seq_assoc)
  also have "... = (atomic p \<otimes> atomic q) ; (((atomic p) \<otimes> (atomic q)) \<^sup>;^ i) ; (c \<otimes> d)"
    by (metis Suc.hyps seq_assoc)
  finally show ?case
    by simp
qed

lemma atomic_fiter_sync: "(atomic p \<^sup>;^ i) \<otimes> (atomic q \<^sup>;^ i) = (atomic p \<otimes> atomic q) \<^sup>;^ i"
  by (metis nil_sync_nil seq_nil_right sync_fixed_iter)

lemma unmatched_iter_finite:
  assumes "i < j"
  assumes a_def[simp]: "a = atomic p"
  assumes b_def[simp]: "b = atomic q"
  shows "(a \<^sup>;^ i) \<otimes> (b \<^sup>;^ j) = ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
proof -
  have "(a \<^sup>;^ i) \<otimes> (b \<^sup>;^ j) = (a \<^sup>;^ i) ; nil \<otimes> (b \<^sup>;^ i) ; (b \<^sup>;^ (j - i))"
    using assms(1) seq_power_split_less by force
  also have "... = ((a \<otimes> b) \<^sup>;^ i) ; (nil \<otimes> (b \<^sup>;^ (j - i)))"
    by (metis a_def b_def sync_fixed_iter)
  finally show ?thesis
    by (metis assms(1) b_def diff_is_0_eq gr0_implies_Suc less_le_not_le nil_sync_atomic_pre not_gr0 seq_power_Suc)
qed

lemma unmatched_iter_infinite:
  assumes a_def[simp]: "a = atomic p"
  assumes b_def[simp]: "b = atomic q"
  shows "(a \<^sup>;^ i) \<otimes> b\<^sup>\<infinity> = ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
proof -
  have "(a \<^sup>;^ i) \<otimes> b\<^sup>\<infinity> = (a \<^sup>;^ i) ; nil \<otimes> (b \<^sup>;^ i) ; b\<^sup>\<infinity>"
    using infiter_unfold_any by auto
  also have "... = ((a \<otimes> b) \<^sup>;^ i) ; (nil \<otimes>  b\<^sup>\<infinity>)"
    by (metis a_def b_def sync_fixed_iter)
  finally show ?thesis
    by (metis b_def infiter_unfold nil_sync_atomic_pre)
qed

lemma omega_sync_helper:
  assumes "j < i"
  assumes a_def[simp]: "a = atomic p"
  assumes b_def[simp]: "b = atomic q"
  shows "((a \<otimes> b) \<^sup>;^ i) \<ge> ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>"
  using assms(1) seq_mono_right seq_power_split_less by auto

(* The case where i = infinity is given by inf_subsumes in AbstractAtomicTest/Abstract_Atomic_Sync_Test.thy *)
lemma omega_sync_finite:
  assumes a_def[simp]: "a = atomic p"
  assumes b_def[simp]: "b = atomic q"
  shows "a\<^sup>\<omega> \<otimes> (b \<^sup>;^ i) = ((a \<otimes> b) \<^sup>;^ i)"
proof -
  define f where f_def: "f \<equiv> \<lambda> j. (a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i)" 
  define j_less_i where jli_def: "j_less_i = {j::nat. j < i}"
  define j_greater_i where jgi_def: "j_greater_i = {j::nat. j > i}"
  have nat_split: "\<forall>j::nat. j \<in> {i} \<union> j_less_i \<union> j_greater_i"
    using jgi_def jli_def by auto
  have nat_split_2: "(\<Squnion>j::nat. f(j)) = (\<Squnion>j \<in> {i} \<union> j_less_i \<union> j_greater_i. f(j))"
    by (metis UNIV_eq_I nat_split)
  have nat_split_3: "(\<Squnion>j::nat. f(j)) = f(i) \<squnion> (\<Squnion>j\<in>j_less_i. f(j)) \<squnion> (\<Squnion>j\<in>j_greater_i. f(j))"
    by (metis SUP_insert SUP_union insert_is_Un nat_split_2)
  
  have i_ref_magic: "((a \<otimes> b) \<^sup>;^ i) \<ge> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    by (metis iter1 nil_iter seq_mono_right seq_nil_right)
  have i_ref_jli: "((a \<otimes> b) \<^sup>;^ i) \<ge> (\<Squnion>j\<in>{j::nat. j < i}. ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>)"
    by (simp add: SUP_le_iff omega_sync_helper)

  have "a\<^sup>\<omega> \<otimes> (b \<^sup>;^ i) = (\<Squnion>j::nat. a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i) \<squnion> a\<^sup>\<infinity> \<otimes> (b \<^sup>;^ i)"
    by (simp add: fiter_seq_choice iter_isolation nondet_sync_distrib)
  also have "... = (\<Squnion>j::nat. (a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i)) \<squnion> a\<^sup>\<infinity> \<otimes> (b \<^sup>;^ i)"
    by (simp add: NONDET_sync_distrib)
  also have "... = (\<Squnion>j::nat. (a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i)) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    using sync_commute unmatched_iter_infinite by auto
  also have "... = (\<Squnion>j::nat. f(j)) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    by (metis f_def)
  also have "... = f(i) \<squnion> (\<Squnion>j\<in>j_less_i. f(j)) \<squnion> (\<Squnion>j\<in>j_greater_i. f(j)) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    by (simp add: nat_split_3)
  also have "... = (a \<^sup>;^ i) \<otimes> (b \<^sup>;^ i) \<squnion> (\<Squnion>j\<in>{j::nat. j < i}. (a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i)) \<squnion> (\<Squnion>j\<in>{j::nat. j > i}. (a \<^sup>;^ j) \<otimes> (b \<^sup>;^ i)) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    using f_def jgi_def jli_def by fastforce
  also have "... = (a \<^sup>;^ i) \<otimes> (b \<^sup>;^ i) \<squnion> (\<Squnion>j\<in>{j::nat. j < i}. ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>) \<squnion> (\<Squnion>j\<in>{j::nat. j > i}. ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    using unmatched_iter_finite a_def b_def sync_commute by (smt (verit) Inf.INF_cong mem_Collect_eq) 
  also have "... = ((a \<otimes> b) \<^sup>;^ i) \<squnion> (\<Squnion>j\<in>{j::nat. j < i}. ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>) \<squnion> (\<Squnion>j\<in>{j::nat. j > i}. ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    by (metis atomic_fiter_sync a_def b_def)
  also have "... = ((a \<otimes> b) \<^sup>;^ i) \<squnion> (\<Squnion>j\<in>{j::nat. j < i}. ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>) \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom> \<squnion> ((a \<otimes> b) \<^sup>;^ i) ; \<bottom>"
    by (smt (verit, best) SUP_bot_conv(1) SUP_eq_const empty_Collect_eq mem_Collect_eq sup.right_idem sup_bot.right_neutral)
  also have "... = ((a \<otimes> b) \<^sup>;^ i) \<squnion> (\<Squnion>j\<in>{j::nat. j < i}. ((a \<otimes> b) \<^sup>;^ j) ; \<bottom>)"
    by (metis (no_types, lifting) i_ref_magic sup.coboundedI1 sup_absorb1)
  finally show ?thesis
    by (metis i_ref_jli sup.orderE)
qed
...
lemma iter_or_abort_alt:
  assumes atomic_a[simp]: "a = (atomic p)"
  assumes atomic_b[simp]: "b = (atomic q)"
  shows "(a \<squnion> b ; \<top>)\<^sup>\<omega> = a\<^sup>\<omega> ; (nil \<squnion> b ; \<top>)"
proof -
  have "(a \<squnion> b ; \<top>)\<^sup>\<omega> =  a\<^sup>\<omega> ; (b ; \<top> ;  a\<^sup>\<omega>)\<^sup>\<omega>"
    by (simp add: iter_decomp)
  also have "...  =  a\<^sup>\<omega> ; (b ; \<top>)\<^sup>\<omega>"
    by (simp add: seq_assoc)
  also have "... = a\<^sup>\<omega> ; (nil \<squnion> (b ; \<top>) ; (b ; \<top>)\<^sup>\<omega>)"
