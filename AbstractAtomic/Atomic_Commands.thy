...
lemma atomic_inf_conj_distrib_seq:
  ...
  shows "a\<^sup>\<omega> \<iinter> c;d = (a\<^sup>\<omega> \<iinter> c); (a\<^sup>\<omega> \<iinter> d)" 
(* proof using the canonical representation *)
*)

lemma atomic_iter_intro:
  assumes atomic_a[simp]: "a = (atomic p)"
  shows "c \<ge> a\<^sup>\<omega> \<iinter> c"
proof -
  have atomic_ref_a: "Atomic \<ge> a"
    by (simp add: atomic.Hom_def atomic.hom_mono)
  have iter_atomic_ref_a: "Atomic\<^sup>\<omega> \<ge> a\<^sup>\<omega>"
    by (metis atomic_ref_a iter_mono)
  have "c = chaos \<iinter> c"
    by (simp add: conj_commute)
  also have "... = Atomic\<^sup>\<omega> \<iinter> c"
    by (simp add: chaos_def)
  also have "... \<ge> a\<^sup>\<omega> \<iinter> c"
    by (metis conj.sync_mono_left iter_atomic_ref_a)
  finally show ?thesis .
qed
