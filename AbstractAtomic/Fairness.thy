...
text \<open>Define the command fair and the fair parallel operator. 
  We add two additional strong interchange axioms 
  which assume d0 and d1 do not abort immediately and 
  select the traces of c0 and c1 that take exactly k steps for the interchange.\<close>
...
(* The following axioms should be moved elsewhere *)
 assumes atomic_distrib_par: "(nil \<squnion> Atomic;\<top> \<ge> d0) \<and> (nil \<squnion> Atomic;\<top> \<ge> d1) \<longrightarrow> 
      ((c0 \<iinter> (Atomic\<^sup>;^k));d0 \<parallel> (c1 \<iinter> (Atomic\<^sup>;^k));d1 = ((c0 \<iinter> (Atomic\<^sup>;^k)) \<parallel> (c1 \<iinter> (Atomic\<^sup>;^k))); (d0 \<parallel> d1))"
 assumes atomic_distrib_conj: "(nil \<squnion> Atomic;\<top> \<ge> d0) \<and> (nil \<squnion> Atomic;\<top> \<ge> d1) \<longrightarrow> 
      ((c0 \<iinter> (Atomic\<^sup>;^k));d0 \<iinter> (c1 \<iinter> (Atomic\<^sup>;^k));d1 = (c0 \<iinter> c1 \<iinter> (Atomic\<^sup>;^k)); (d0 \<parallel> d1))"
...
(* This lemma should go in a more appropriate theory *)
lemma decompose_finite: "c \<iinter> Atomic\<^sup>\<star> = (\<Squnion>k::nat. c \<iinter> (Atomic\<^sup>;^k))" (is "?lhs = ?conc")
proof -
  have a1: "?lhs = c \<iinter> (\<Squnion>k::nat. (Atomic\<^sup>;^k))"
    by (simp add: par.fiter_seq_choice)
  also have a2: "\<dots> =  (\<Squnion>k::nat. c \<iinter> (Atomic\<^sup>;^k))"
    using conj.sync_Nondet_distrib empty_not_UNIV
    by (metis conj.sync_NONDET_distrib)
  thus ?thesis using a1 a2
    by presburger 
qed

(* This lemma should go in a more appropriate theory *)
lemma decompose_atomic: "c = (\<Squnion>k::nat. c \<iinter> (Atomic\<^sup>;^k)) \<squnion> (c \<iinter> Atomic\<^sup>\<infinity>)"
proof -
  have a0: "c = c \<iinter> Atomic\<^sup>\<omega>"
    using chaos_def conj_id by simp 
  also have a1: "\<dots> = c \<iinter> Atomic\<^sup>\<star> \<squnion> c \<iinter> Atomic\<^sup>\<infinity>" 
    by (simp add: conj.sync_nondet_distrib par.iter_isolation)
  also have a2: "\<dots> = c \<iinter> (\<Squnion>k::nat. Atomic\<^sup>;^k) \<squnion> c \<iinter> Atomic\<^sup>\<infinity>"
    using par.fiter_seq_choice by presburger
  also have a3: "\<dots> = (\<Squnion>k::nat. c \<iinter> (Atomic\<^sup>;^k)) \<squnion> c \<iinter> Atomic\<^sup>\<infinity>"
    using decompose_finite par.fiter_seq_choice by force 
  thus ?thesis using calculation by presburger 
qed

lemma decompose_parallel: 
  "c \<parallel> d = (\<Squnion>k::nat. c \<iinter> (Atomic\<^sup>;^k) \<parallel> d \<iinter> (Atomic\<^sup>;^k)) \<squnion> (c \<iinter> Atomic\<^sup>\<infinity> \<parallel> d \<iinter> (Atomic\<^sup>\<infinity>))" (is "?lhs = ?conc")
proof -
  have a1: "?lhs = (\<Squnion>k::nat. (c \<parallel> d) \<iinter> (Atomic\<^sup>;^k)) \<squnion> ((c \<parallel> d) \<iinter> Atomic\<^sup>\<infinity>)"
    using decompose_atomic by simp 
  have a2: "\<dots> = (\<Squnion>k::nat. (c \<iinter> (Atomic\<^sup>;^k) \<parallel> d \<iinter> (Atomic\<^sup>;^k))) \<squnion> ((c \<parallel> d) \<iinter> Atomic\<^sup>\<infinity>)"
    sorry
  thus ?thesis sorry
qed

(* should be generalised to sync and moved there *)
lemma par_termination:
  assumes a_atomic: "Atomic \<ge> a"
  assumes b_atomic: "Atomic \<ge> b"
  assumes c_term: "c = c \<iinter> Atomic\<^sup>\<star>"
  assumes d_term: "d = d \<iinter> Atomic\<^sup>\<star>"
  shows "(c;a\<^sup>\<star> \<parallel> d;b\<^sup>\<star>);(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>) = c;a\<^sup>\<omega> \<parallel> d;b\<^sup>\<omega>"
proof -
  have aomega: "a\<^sup>\<star>;a\<^sup>\<omega> = a\<^sup>\<omega>" using fiter_seq_iter by simp
  have bomega: "b\<^sup>\<star>;b\<^sup>\<omega> = b\<^sup>\<omega>" using fiter_seq_iter by simp
  have r2l: "c;a\<^sup>\<omega> \<parallel> d;b\<^sup>\<omega> \<ge> (c;a\<^sup>\<star> \<parallel> d;b\<^sup>\<star>);(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>)"
  proof -
    have a: "c;a\<^sup>\<omega> \<parallel> d;b\<^sup>\<omega> = c;a\<^sup>\<star>;a\<^sup>\<omega> \<parallel> d;b\<^sup>\<star>;b\<^sup>\<omega>" using aomega bomega
      by (simp add: seq_assoc)
    also have b: "\<dots> \<ge> (c;a\<^sup>\<star> \<parallel> d;b\<^sup>\<star>);(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>)" using a
      by (simp add: seq_par_interchange)
    show ?thesis
      by (simp add: b calculation)
  qed
  have l2r: "(c;a\<^sup>\<star> \<parallel> d;b\<^sup>\<star>);(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>) \<ge> c;a\<^sup>\<omega> \<parallel> d;b\<^sup>\<omega>"
  proof -
    have "(c;a\<^sup>\<star> \<parallel> d;b\<^sup>\<star>);(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>) \<ge> 
        (\<Squnion>k::nat. ((c;a\<^sup>\<star> \<iinter> (Atomic\<^sup>;^k)) \<parallel> (d;b\<^sup>\<star> \<iinter> (Atomic\<^sup>;^k)));(a\<^sup>\<omega> \<parallel> b\<^sup>\<omega>))"
      sorry
    show ?thesis sorry
  qed
  show ?thesis using r2l l2r
    using antisym sorry 
qed
...
proof -
  ...
  have a5: "?rhs \<ge> (c \<iinter> Atomic\<^sup>\<star> \<iinter> fair);skip \<parallel> (d \<iinter> Atomic\<^sup>\<star> \<iinter> fair);skip"
    using skip_def par_termination
  proof -
    obtain pp :: "'c \<times> 'c" where
      f1: "\<E> = atomic pp"
      using par.syncid_atomic by blast
    then have "(c \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; skip \<parallel> (d \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; skip = (c \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; atomic pp\<^sup>\<omega> \<parallel> (d \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; atomic pp\<^sup>\<omega>"
      using skip_def by force
    then have "(c \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; skip \<parallel> (d \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; skip = ((c \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; atomic pp\<^sup>\<star> \<parallel> (d \<iinter> Atomic\<^sup>\<star> \<iinter> fair) ; atomic pp\<^sup>\<star>) ; (atomic pp\<^sup>\<omega> \<parallel> atomic pp\<^sup>\<omega>)"
      using atomic.Hom_ref_hom local.conj_assoc local.conj_commute par_termination by auto
    then show ?thesis
      using f1 par.syncid_syncid_iter skip_def by force 
  qed 
...
(* subsumed
lemma introduce_fair_skip: "c \<parallel>\<^sub>f d \<ge> ((c \<parallel>\<^sub>f d) \<iinter> fair);skip"
...
lemma fourway_nondet: 
  assumes cd1: "(c1::'a) \<ge> d1"
  assumes cd2: "(c2::'a) \<ge> d2"
  assumes cd3: "(c3::'a) \<ge> d3"
  assumes cd4: "(c4::'a) \<ge> d4"
  shows "c1 \<squnion> c2 \<squnion> c3 \<squnion> c4 \<ge> d1 \<squnion> d2 \<squnion> d3 \<squnion> d4"
  by (meson cd1 cd2 cd3 cd4 sup.mono)
...
proof - 
  ...
  proof -
    ...
    also have "\<dots> \<ge> (c \<iinter> Atomic\<^sup>\<star> \<parallel>\<^sub>f d \<iinter> Atomic\<^sup>\<star>) \<squnion> 
      ...
      using fourway_nondet f_f f_i i_f i_i  by blast
    finally show ?thesis 
...
lemma interchange: "(c0;c1) \<parallel> (d0;d1) \<ge> (c0 \<parallel> d0);(c1 \<parallel> d1)"
  by (simp add: par.seq_conj_interchange)
  
theorem fair_par_interchange_seq: 
  "(Atomic\<^sup>\<star>;Atomic\<^sup>\<infinity>) \<parallel>\<^sub>f (Atomic\<^sup>\<infinity>;Atomic\<^sup>\<star>) \<ge> (Atomic\<^sup>\<star> \<parallel>\<^sub>f  Atomic\<^sup>\<infinity>);(Atomic\<^sup>\<infinity> \<parallel>\<^sub>f Atomic\<^sup>\<star>)"
  oops
...
lemma fair_pseudo_atomic_fixed_point: "fair = (\<alpha>(\<top>) \<squnion> \<alpha>(\<bottom>) ; \<top>) ; fair \<squnion> nil"
proof -
  have "fair = \<E>\<^sup>\<star> ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>"
    by (metis fair_def)
  also have "... = (nil \<squnion> \<E> ; \<E>\<^sup>\<star>) ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>"
    by (metis fiter_unfold)
  also have "... = (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega> \<squnion> \<E> ; \<E>\<^sup>\<star> ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>"
    by (simp add: nondet_seq_distrib)
  also have "... = nil \<squnion> Pgm ; (\<E>\<^sup>\<star> ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>) \<squnion> \<E> ; (\<E>\<^sup>\<star> ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>)"
    by (metis iter_unfold seq_assoc)
  also have "... = nil \<squnion> (Pgm \<squnion> \<E>) ; (\<E>\<^sup>\<star> ; (Pgm ; \<E>\<^sup>\<star>)\<^sup>\<omega>)"
    by (simp add: nondet_seq_distrib sup_assoc)
  also have "... = \<alpha>(\<top>) ; fair \<squnion> nil"
    by (simp add: Env_def Pgm_def fair_def general_atomic_def inf_sup_aci(5))
  also have "... = (\<alpha>(\<top>) \<squnion> \<alpha>(\<bottom>) ; \<top>) ; fair \<squnion> nil"
    by simp
  finally show ?thesis .
qed

lemma distrib_seq_par_fair: "fair \<parallel> (c ; d) = (fair \<parallel> c) ; (fair \<parallel> d)"
  using fair_pseudo_atomic_fixed_point par.atomic_iter_distrib_seq
  by (metis Atomic_def Atomic_def2 Env_def Pgm_def atomic.hom_bot general_atomic.hom_bot general_atomic_def) 

lemma distrib_seq_conj_fair: "fair \<iinter> (c ; d) = (fair \<iinter> c) ; (fair \<iinter> d)"
  using fair_pseudo_atomic_fixed_point conj.atomic_iter_distrib_seq
  by (metis Atomic_def Atomic_def2 Env_def Pgm_def atomic.hom_bot general_atomic.hom_bot general_atomic_def)

lemma pseudo_distrib_finite_iter_par_fair: 
  assumes "c \<parallel> nil = \<bottom>"
  shows "fair \<parallel> c\<^sup>\<star> = (fair \<parallel> c)\<^sup>\<star>"
  by (metis Atomic_def Atomic_def2 Env_def Pgm_def atomic.hom_bot par.distrib_finite_iter2 general_atomic.hom_bot general_atomic_def fair_pseudo_atomic_fixed_point)

lemma pseudo_distrib_finite_iter_conj_fair: 
  assumes "c \<iinter> nil = \<bottom>"
  shows "fair \<iinter> c\<^sup>\<star> = (fair \<iinter> c)\<^sup>\<star>"
  by (metis Atomic_def Atomic_def2 Env_def Pgm_def atomic.hom_bot conj.distrib_finite_iter2 general_atomic.hom_bot general_atomic_def fair_pseudo_atomic_fixed_point)
