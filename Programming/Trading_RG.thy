...
theory Trading_RG
imports
  "../AbstractAtomicTest/Relies_Guarantees"    
  "Specification"
begin
...
lemma pre_spec_trading: "(rely r) \<iinter> (guar g) \<iinter> \<lbrace>p\<rbrace>;\<lparr>(r \<squnion> g)\<^sup>* \<sqinter> q\<rparr> = (rely r) \<iinter> (guar g) \<iinter> \<lbrace>p\<rbrace>;\<lparr>q\<rparr>"
  using spec_trading by (metis conj.assert_distrib)
