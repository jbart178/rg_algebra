...
imports
  ...
  "../AbstractAtomicTest/Termination"
...
lemma spec_ref_opt_pre:
  assumes "p \<triangleleft> q2 \<subseteq> q1"
  shows "\<lbrace>p\<rbrace>;\<lparr>q1\<rparr> \<ge> \<lbrace>p\<rbrace>;opt q2"
  using spec_ref_opt by (meson assms dual_order.trans opt_strengthen_under_pre seq_mono_right) 

text \<open>Refine a specification command in the context of a guarantee and 
  precondition to an optional atomic step command.
  The guarantee must be reflexive to allow for the optional.\<close>

lemma spec_ref_opt_guar:
  assumes refl_g: "refl g"
  assumes strengthen: "p \<triangleleft> q2 \<subseteq> g \<inter> q1"
  shows "guar g \<iinter> \<lbrace>p\<rbrace>;\<lparr>q1\<rparr> \<ge> opt q2"
proof -
  have a: "guar g \<iinter> \<lbrace>p\<rbrace>;\<lparr>q1\<rparr> \<ge> guar g \<iinter> \<lbrace>p\<rbrace>;opt q1" (is "_ \<ge> ?rhs")
    using  conj.sync_mono_right seq_mono_right spec_ref_opt by presburger
  have b: "?rhs = \<lbrace>p\<rbrace>;(guar g \<iinter> opt q1)"
    by (simp add: conj.assert_distrib)
  have c: "\<dots> \<ge> opt(q2)"
    using guar_opt refl_g opt_strengthen_under_pre strengthen assert_redundant 
    by auto 
  show ?thesis using a b c by force
qed
