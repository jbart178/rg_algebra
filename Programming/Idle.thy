...
lemma idle_pseudo_atomic_fixed_point: 
  shows "idle = (atomic(Id, \<top>) \<squnion> atomic(\<bottom>) ; \<top>) ; idle \<squnion> nil"
proof -
  have "idle = ((atomic(Id, \<top>) \<squnion> atomic \<bottom> ; \<top>) ; guar\<^sub>\<pi> Id \<squnion> nil) \<iinter> ((atomic(\<top>) \<squnion> atomic(\<bottom>) ; \<top>) ; term \<squnion> nil)"
    using Atomic_def Atomic_def2 Env_def Pgm_def general_atomic_def idle_def psuedo_atomic_fixed_point_guar_pgm term_pseudo_atomic_fixed_point by auto
  also have "... = (atomic ((Id, \<top>) \<sqinter> \<top>) \<squnion> atomic (((Id, \<top>) \<sqinter> \<bottom>) \<squnion> (\<top> \<sqinter> \<bottom>) \<squnion> (\<bottom> \<sqinter> \<bottom>)) ; \<top>) ; (guar\<^sub>\<pi> Id \<iinter> term) \<squnion> nil"
    by (metis Atomic_def Atomic_def2 Env_def Pgm_def atomic.hom_bot boolean_algebra.conj_one_right boolean_algebra.conj_zero_right conjoin_pseudo_atomic_fixed_points general_atomic.hom_bot general_atomic_def psuedo_atomic_fixed_point_guar_pgm term_pseudo_atomic_fixed_point)
  also have "... = (atomic (Id, \<top>) \<squnion> atomic ((\<bottom>) \<squnion> (\<bottom>) \<squnion> (\<bottom>)) ; \<top>) ; (guar\<^sub>\<pi> Id \<iinter> term) \<squnion> nil"
    by (simp add: boolean_algebra.conj_one_right)
  also have "... = (atomic(Id, \<top>) \<squnion> atomic(\<bottom>) ; \<top>) ; idle \<squnion> nil"
    by (simp add: idle_def)
  finally show ?thesis .
qed

lemma distrib_seq_par_idle: "idle \<parallel> (c ; d) = (idle \<parallel> c) ; (idle \<parallel> d)"
  by (metis idle_pseudo_atomic_fixed_point par.atomic_iter_distrib_seq)

lemma distrib_seq_conj_idle: "idle \<iinter> (c ; d) = (idle \<iinter> c) ; (idle \<iinter> d)"
  by (metis idle_pseudo_atomic_fixed_point conj.atomic_iter_distrib_seq)

lemma pseudo_distrib_finite_iter_par_idle: 
  assumes "c \<parallel> nil = \<bottom>"
  shows "idle \<parallel> c\<^sup>\<star> = (idle \<parallel> c)\<^sup>\<star>"
  by (metis par.distrib_finite_iter2 idle_pseudo_atomic_fixed_point)

lemma pseudo_distrib_finite_iter_conj_idle: 
  assumes "c \<iinter> nil = \<bottom>"
  shows "idle \<iinter> c\<^sup>\<star> = (idle \<iinter> c)\<^sup>\<star>"
  by (metis conj.distrib_finite_iter2 idle_pseudo_atomic_fixed_point)
...
lemma rely_idle_stable_assert:
  assumes stable_p: "stable p r"
  shows "(rely r) \<iinter> \<lbrace>p\<rbrace>;idle \<ge> (rely r) \<iinter> idle ; \<lbrace>p\<rbrace>"
proof -
  have a: "(rely r) \<iinter> \<lbrace>p\<rbrace>;idle \<ge> (rely r) \<iinter> \<lbrace>p\<rbrace>; idle; \<tau> p; \<lbrace>p\<rbrace>" (is "_ \<ge> ?rhs")
    using conj.sync_mono_right nil_ref_test seq_mono_right seq_nil_right 
    by (metis seq_assoc test_seq_assert)
  then have b: "?rhs \<ge> (rely r) \<iinter> \<lbrace>p\<rbrace>; \<tau> p; idle ; \<lbrace>p\<rbrace>" (is "_ \<ge> ?rhs")
    by (metis rely_idle_stable rely_refine_within seq_assoc stable_p)
  then have c: "?rhs \<ge> (rely r) \<iinter> idle ; \<lbrace>p\<rbrace>" 
    using rely_refine_within assert_seq_test assert_redundant conj.sync_mono_right seq_assoc
      assert_remove_left by presburger 
  then show ?thesis using a b c
    by simp
qed
