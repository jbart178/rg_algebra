...
proof -
  ...
  proof -
    have "(x ::= e) = (\<Squnion>k::nat. \<lbrakk>e\<rbrakk>k ; opt(update x k) ; idle)" 
      unfolding assign_def by simp
    also have b: "... = (\<Squnion>k::nat. \<lbrakk>e\<rbrakk>k ; opt(update x k) ; idle ; idle)" 
      using idle_seq_idle seq_assoc by auto
    also have "... = (\<Squnion>k::nat. \<lbrakk>e\<rbrakk>k ; opt(update x k) ; idle)  ; idle" 
      by (simp add: NONDET_seq_distrib) 
    also have "... \<ge> (\<Squnion>k::nat. (idle ; \<lbrakk>e\<rbrakk>k) ; opt(update x k) ; idle)  ; idle" (is "_ \<ge> ?rhs")
      by (simp add: NONDET_mono_quant a seq_mono_left)
    also have "?rhs \<ge> (\<Squnion>k::nat. idle ; (\<lbrakk>e\<rbrakk>k ; opt(update x k) ; idle))  ; idle" (is "_ \<ge> ?rhs") 
      using seq_assoc by auto
    also have "?rhs \<ge> idle ; (\<Squnion>k::nat. \<lbrakk>e\<rbrakk>k ; opt(update x k) ; idle)  ; idle" 
